...
async function bootstrap() {
  try {
    ...
    if (!exists) {
      ...
      if (error) console.log(`Bootstrap error creating admin user: ${error.message}`);
    }
  } catch (err) {
  ...
  try {
    ...
    if (!seededFlag) {
      await kv.mset(seedProjects.map((p) => `project:${p.id}`), seedProjects);
      await kv.mset(seedTestimonials.map((t) => `testimonial:${t.id}`), seedTestimonials);
...
function ensureBootstrap(): Promise<void> {
  ...
  if (!bootstrapPromise) {
    bootstrapPromise = bootstrap()
      ...
      .catch((err) => { console.log(`Bootstrap failed: ${err}`); bootstrapPromise = null; });
  }
...
app.get(`${PREFIX}/health`, (c) => c.json({ status: "ok", version: VERSION }));
...
app.get(`${PREFIX}/content`, async (c) => {
  try {
    ...
    return c.json({ error: `Failed to fetch content: ${err}` }, 500);
  }
});
...
app.put(`${PREFIX}/content`, async (c) => {
  ...
  try {
    ...
    return c.json({ error: `Failed to save content: ${err}` }, 500);
  }
});
...
app.post(`${PREFIX}/projects`, async (c) => {
  ...
  try {
    ...
    const project = { id, title: body.title, tags: body.tags || "", status: body.status || "Draft", views: 0, description: body.description || "" };
...
app.get(`${PREFIX}/testimonials`, async (c) => {
  try {
    ...
    return c.json({ error: `Failed to fetch testimonials: ${err}` }, 500);
  }
});
...
app.post(`${PREFIX}/testimonials`, async (c) => {
  ...
  try {
    const body = await c.req.json();
    const id = String(Date.now());
    const testimonial = { id, name: body.name, role: body.role || "", rating: body.rating ?? 5, status: body.status || "Published", quote: body.quote || "" };
    await kv.set(`testimonial:${id}`, testimonial);
    return c.json({ testimonial });
  } catch (err) {
    console.log(`Error creating testimonial: ${err}`);
    return c.json({ error: `Failed to create testimonial: ${err}` }, 500);
  }
});
...
app.put(`${PREFIX}/testimonials/:id`, async (c) => {
  ...
  try {
    ...
    return c.json({ error: `Failed to update testimonial: ${err}` }, 500);
  }
});
...
app.delete(`${PREFIX}/testimonials/:id`, async (c) => {
  ...
  try {
    ...
    return c.json({ error: `Failed to delete testimonial: ${err}` }, 500);
  }
});
